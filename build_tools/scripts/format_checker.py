...
fc_path = os.path.abspath(sys.path[0] + "../../..")
...
class Header(object):
    ...
    def __init__(self, path, desc):
        ...
        if path.endswith(".cpp") or path.endswith(".cc"):
            ...
            sys.exit(0)
        if not self.__path.startswith(fc_path):
            logging.info("Skip: format_checker is only for fc file")
            sys.exit(0)
        # Gen header.
            ...
            "-", "\-") + ".*" + tail.replace("*", "\*").replace(
                "-", "\-").replace("+", "\+") + copyright.replace(
                    "(", "\(").replace(")", "\)").replace("[", "\[").replace(
                        "]", "\]").replace("-", "\-")
        if self.__has_macro:
            file_macro = self.__path.replace(fc_path + "/", "").replace(
                "/", "_").replace(".", "_").upper()
...
if __name__ == "__main__":
    ...
    check = args.check
    if check == "True" and not os.path.exists(args.path):
